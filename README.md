...
- **Category B — Uncorrelated**: |r| < threshold + not significant
...
main.py                # orchestrates pipeline end-to-end
requirements.txt
```
...
- Outputs: [`output/classified_coins.csv`](output/classified_coins.csv) and [`output/scatter_beta_corr.png`](output/scatter_beta_corr.png).
...
No threshold is hardcoded in `src/` — all read from config.
