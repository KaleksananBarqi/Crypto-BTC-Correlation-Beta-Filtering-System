...
def load_config(path: str = "config.yaml") -> Dict:
    ...
    # Basic validation
    assert "thresholds" in cfg, "config.yaml missing 'thresholds'"
    assert "windows" in cfg, "config.yaml missing 'windows'"
    return cfg
...
def generate_synthetic_demo_data(cfg: Dict) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    """
...
def main() -> None:
    ...
    try:
        ...
        if cached_btc is not None and not cached_btc.empty and cached_alts:
            ...
            btc_df, alt_map = generate_synthetic_demo_data(cfg)

    assert btc_df is not None and not btc_df.empty, "BTC data is empty — cannot proceed"
    assert alt_map, "No alt data — cannot proceed"
    ...
    # ── 4. Metrics (parallel) ───────────────────────────────────────────
    # compute_all_coins_metrics expects Dict[str, Tuple[btc_series, alt_series]]
    dummy_btc = pd.Series(dtype=float)  # not used when map values are tuples
    all_metrics = compute_all_coins_metrics(
        btc_returns=dummy_btc,
